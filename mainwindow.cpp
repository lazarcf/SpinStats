...
void MainWindow::on_databaseCombo_currentIndexChanged(const QString &db_name)
{
    ...
    if (!m_db.record("player").contains("custom_type")) {
        if (QMessageBox::question(this, "Dialog", "Did not find 'custom_type' column in your selected database, add it? Without this column nothing will work. Please backup before going any further.") ==
                QMessageBox::Yes) {
    ...
    if (database.tournies.count() == 0) {
        ...
        if (!q.exec("SELECT id_table_type FROM tourney_table_type WHERE description='STT (3 max) Hyper-Turbo SNG LOTTERY'")) {
...
void MainWindow::on_regLoadButton_clicked()
{
    ...
    QSqlQuery q;
    int count = 0;
    foreach (auto key, regs.keys()) {
        q.prepare("UPDATE player SET custom_type=? WHERE player_name_search=?");
        q.addBindValue(regs [key].toLower());
        q.addBindValue(key.toLower());
        if (q.exec())
            count += 1;
